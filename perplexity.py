...
def perform_perplexity_research(query: str, 
    ...
    payload = {
        "model": model,
        "messages": [
            {"role": "user", "content": f"Conduct detailed research on the following topic. Provide comprehensive information with sources. Topic: {query}"}
        ],
...
def extract_sources_from_content(content: str) -> List[Dict[str, str]]:
    ...
    for i, line in enumerate(lines):
        ...
        if (line.strip().startswith("[") and "]" in line) or \
           "Source:" in line or "Reference:" in line or \
           "http://" in line or "https://" in line:
            ...
            if url_start != -1:
                ...
                title = title.replace("[", "").replace("]", "").replace("Source:", "").replace("Reference:", "").strip()
                
                if not title and i > 0:
                    title = lines[i-1].strip()
                
                sources.append({
                    "title": title if title else "Unknown Source",
                    "url": url
                })
